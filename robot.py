...
def auto_reply_text_to_firends(msg):
    ...
    if (res_data['type'] == 'add_member'):
        ...
        if (res_data['currentGroup'] == 'HQGroup'):
            sender.send('加入大群成功,请输入姓名')
        if (res_data['currentGroup'] == 'CityGroup'):
            sender.send('加入城市群成功,请输入完整的学校名称')
        if (res_data['currentGroup'] == 'CollegeGroup'):
            sender.send('加入学校群完毕')
        if(res_data['currentGroup'] =='ConsultGroup'):
            sender.send('请点击链接加入咨询群')
        if len(gs) == 0:
